...
use super::{HeaderName, HeaderValue, InvalidHeaderName};
...
///
/// headers.insert(Expires, "Wed, 23 Jan 2013 15:36:52 +0000".try_into().unwrap());
/// headers.insert(ContentLength, "123".try_into().unwrap());
///
/// assert!(headers.contains_key(Expires));
/// assert!(!headers.contains_key(Location));
///
/// assert_eq!(headers[Expires], "Wed, 23 Jan 2013 15:36:52 +0000");
///
...
//
// Below, you will find a fairly large amount of code. Most of this is to provide the necessary
// functions to efficiently manipulate the header multimap. The core hashing table is based on robin
// hood hashing [1]. While this is the same hashing algorithm used as part of Rust's `HashMap` in
// stdlib, many implementation details are different. The two primary reasons for this divergence
// are that `HeaderMap` is a multimap and the structure has been optimized to take advantage of the
// characteristics of RTSP headers.
//
// ## Structure Layout
//
// Most of the data contained by `HeaderMap` is *not* stored in the hash table. Instead, pairs of
// header name and *first* associated header value are stored in the `entries` vector. If the header
// name has more than one associated header value, then additional values are stored in
// `extra_values`. The actual hash table (`indices`) only maps hash codes to indices in `entries`.
// This means that, when an eviction happens, the actual header name and value stay put and only a
// tiny amount of memory has to be copied.
//
// Extra values associated with a header name are tracked using a linked list. Links are formed with
// offsets into `extra_values` and not pointers.
//
// [1]: rtsps://en.wikipedia.org/wiki/Hash_table#Robin_Hood_hashing

/// `HeaderMap` entry iterator.
///
/// Yields `(&HeaderName, &value)` tuples. The same header name may be yielded more than once if it
...
///
/// Yields `(&HeaderName, &mut value)` tuples. The same header name may be yielded more than once if
/// it has more than one associated value.
#[derive(Debug)]
...
///
/// Setting this limit is especially beneficial for `indices`, making it more cache friendly. More
/// hash codes can fit in a cache line.
///
/// You may notice that `u16` may represent more than 32,768 values. This is true, but 32,768 should
/// be plenty and it allows us to reserve the top bit for future usage.
type Size = usize;

/// This limit falls out from above.
const MAX_SIZE: usize = (1 << 15);

/// An entry in the hash table. This represents the full hash code for an entry as well as the
/// position of the entry in the `entries` vector.
#[derive(Copy, Clone)]
struct Pos {
...
/// Hash values are limited to u16 as well. While `fast_hash` and `Hasher` return `usize` hash
/// codes, limiting the effective hash code to the lower 16 bits is fine since we know that the
/// `indices` vector will never grow beyond that size.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
struct HashValue(usize);

/// Stores the data associated with a `HeaderMap` entry. Only the first value is included in this
/// struct. If a header name has more than one associated value, all extra values are stored in the
/// `extra_values` vector. A doubly linked list of entries is maintained. The doubly linked list is
/// used so that removing a value is constant time. This also has the nice property of enabling
...
/// A header value node is either linked to another node in the `extra_values` list or it points to
/// an entry in `entries`. The entry in `entries` is the start of the list and holds the associated
/// header name.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
enum Link {
    Entry(usize),
...
/// Tracks the header map danger level! This relates to the adaptive hashing algorithm. A
/// `HeaderMap` starts in the green state, and when a large number of collisions are detected, it
/// transitions to the yellow state. At this point, the header map will either grow and switch back
/// to the green state, or it will transition to the red state.
///
/// When in the red state, a safe hashing algorithm is used and all values in the header map have to
/// be rehashed.
#[derive(Clone)]
enum Danger {
    Green,
...
// The HeaderMap will use a sequential search strategy until the size of the map exceeds this
// threshold. This tends to be faster for very small header maps. This way all hashing logic can be
// skipped.
const SEQ_SEARCH_THRESHOLD: usize = 8;

// Constants related to detecting DOS attacks.
//
// Displacement is the number of entries that get shifted when inserting a new value. Forward shift
// is how far the entry gets stored from the ideal position.
//
// The current constant values were picked from another implementation. It could be that there are
// different values better suited to the header map case.
const DISPLACEMENT_THRESHOLD: usize = 128;
const FORWARD_SHIFT_THRESHOLD: usize = 512;

// The default strategy for handling the yellow danger state is to increase the header map capacity
// in order to (hopefully) reduce the number of collisions. If growing the hash map would cause the
// load factor to drop below this threshold, then instead of growing, the headermap is switched to
// the red danger state and safe hashing is used instead.
const LOAD_FACTOR_THRESHOLD: f32 = 0.2;

// Macro used to iterate the hash table starting at a given point, looping when the end is hit.
...
// First part of the robinhood algorithm. Given a key, find the slot in which it will be inserted.
// This is done by starting at the "ideal" spot. Then scanning until the destination slot is found.
// A destination slot is either the next empty slot or the next slot that is occupied by an entry
// that has a lower displacement (displacement is the distance from the ideal spot).
//
// This is implemented as a macro instead of a function that takes a closure in order to guarantee
// that it is "inlined". There is no way to annotate closures to guarantee inlining.
    ...
    {{
        ...
        probe_loop!('probe: $probe < $map.indices.len(), {
            if let Some(($pos, entry_hash)) = $map.indices[$probe].resolve() {
                ...
                if their_dist < dist {
                    // The new key's distance is larger, so claim this spot and displace the current
                    // entry.
                    //
...
impl<T> HeaderMap<T> {
    ...
    pub fn with_capacity(capacity: usize) -> HeaderMap<T> {
        ...
        if capacity == 0 {
            ...
            // Avoid allocating storage for the hash table if the requested capacity is below the
            // threshold at which the hash map algorithm is used.
    ...
    ///
    /// This number represents the total number of **values** stored in the map. This number can be
    /// greater than or equal to the number of **keys** stored given that a single key may have more
    /// than one associated value.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// assert_eq!(0, map.len());
    ///
    /// map.insert(Allow, "SETUP, PLAY, SET_PARAMETER".try_into().unwrap());
    /// map.insert(ContentLanguage, "en".try_into().unwrap());
    ///
    /// assert_eq!(2, map.len());
    ///
    ...
    ///
    /// This number will be less than or equal to `len()` as each key may have more than one
    /// associated value.
    ///
    /// # Examples
    ///
    ...
    ///
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    /// map.insert(ContentLanguage, "en".try_into().unwrap());
    ///
    /// assert_eq!(2, map.keys_len());
    ///
    /// map.insert(Allow, "SETUP".try_into().unwrap());
    ///
    /// assert_eq!(2, map.keys_len());
    /// ```
    ...
    /// Reserves capacity for at least `additional` more headers to be inserted into the
    /// `HeaderMap`.
    ///
    ...
    /// let mut map = HeaderMap::new();
    /// map.reserve(10);
    /// # map.insert(Allow, "bar".try_into().unwrap());
    /// ```
    ...
    /// `get_all` to get all values associated with a given key. Returns `None` if there are no
    ...
    ///
    /// let mut map = HeaderMap::new();
    /// assert!(map.get("allow").is_none());
    ///
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    /// assert_eq!(map.get(Allow).unwrap(), &"DESCRIBE");
    /// assert_eq!(map.get("allow").unwrap(), &"DESCRIBE");
    ///
    /// map.append(Allow, "SETUP".try_into().unwrap());
    /// assert_eq!(map.get("allow").unwrap(), &"DESCRIBE");
    /// ```
    ...
    ///
    /// If there are multiple values associated with the key, then the first one is returned. Use
    /// `entry` to get all values associated with a given key. Returns `None` if there are no values
    ...
    /// map.insert(Allow, "GET_PARAMETER".to_string());
    /// map.get_mut("allow").unwrap().push_str(", DESCRIBE");
    ///
    /// assert_eq!(map.get(Allow).unwrap(), &"GET_PARAMETER, DESCRIBE");
    /// ```
    ...
    /// with the key.  See [`GetAll`] for more details. Returns `None` if there are no values
    /// associated with the key.
    ///
    ...
    ///
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    /// map.append(Allow, "GET_PARAMETER".try_into().unwrap());
    ///
    /// let view = map.get_all("allow");
    ///
    /// let mut iter = view.iter();
    /// assert_eq!(&"DESCRIBE", iter.next().unwrap());
    /// assert_eq!(&"GET_PARAMETER", iter.next().unwrap());
    ...
    ///
    /// let mut map = HeaderMap::new();
    /// assert!(!map.contains_key(Allow));
    ///
    /// map.insert(Allow, "world".try_into().unwrap());
    /// assert!(map.contains_key("allow"));
    /// ```
    ...
    ///
    /// The iteration order is arbitrary, but consistent across platforms for the same crate
    /// version. Each key will be yielded once per associated value. So, if a key has 3 associated
    /// values, it will be yielded 3 times.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    /// map.append(Allow, "SETUP".try_into().unwrap());
    /// map.insert(ContentLength, "123".try_into().unwrap());
    ///
    ...
    /// let mut map = HeaderMap::default();
    ///
    /// map.insert(Allow, "DESCRIBE".to_string());
    /// map.append(Allow, "SETUP".to_string());
    /// map.insert(ContentLength, "123".to_string());
    ///
    ...
    ///
    /// The iteration order is arbitrary, but consistent across platforms for the same crate
    /// version. Each key will be yielded only once even if it has multiple associated values.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let mut map = HeaderMap::default();
    ///
    /// map.insert(Allow, "SETUP".to_string());
    /// map.append(Allow, "DESCRIBE".to_string());
    /// map.insert(ContentLength, "123".to_string());
    ///
    ...
    ///
    /// assert_eq!("allow", key);
    /// assert_eq!("SETUP", vals.next().unwrap());
    /// assert_eq!("DESCRIBE", vals.next().unwrap());
    ...
    /// previous values are removed. **Note** that only a single one of the previous values is
    /// returned. If there are multiple values that have been previously associated with the key,
    ...
    ///
    /// let mut map = HeaderMap::new();
    /// assert!(map.insert(Allow, "DESCRIBE".try_into().unwrap()).is_none());
    /// assert!(!map.is_empty());
    ///
    /// let mut prev = map.insert(Allow, "SETUP".try_into().unwrap()).unwrap();
    /// assert_eq!("DESCRIBE", prev);
    /// ```
    ...
    /// map.append(Allow, "SETUP".try_into().unwrap());
    ///
    /// let values = map.get_all("allow");
    /// let mut i = values.iter();
    ///
    /// assert_eq!("DESCRIBE", *i.next().unwrap());
    /// assert_eq!("SETUP", *i.next().unwrap());
    /// ```
    ...
    ///
    /// Returns `None` if the map does not contain the key. If there are multiple values associated
    /// with the key, then the first one is returned. See `remove_entry_mult` on `OccupiedEntry` for
    /// an API that yields all values.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let mut map = HeaderMap::new();
    /// map.insert(Allow, "DESCRIBE, SETUP".try_into().unwrap());
    ///
    /// let prev = map.remove(Allow).unwrap();
    /// assert_eq!("DESCRIBE, SETUP", prev);
    ///
    /// assert!(map.remove(Allow).is_none());
    /// ```
...
impl<T> IntoIterator for HeaderMap<T> {
    ...
    /// Creates a consuming iterator, that is, one that moves keys and values out of the map in
    /// arbitary order. The map cannot be used after calling this.
    ///
    /// For each yielded item that has `None` provided for the `HeaderName`, then the associated
    /// header name is the same as that of the previously yielded item. The first yielded item will
    /// have `HeaderName` set.
    ///
    /// # Examples
    ///
    ...
    /// let mut map = HeaderMap::new();
    /// map.insert(ContentLength, "123".try_into().unwrap());
    /// map.insert(ContentType, "application/sdp".try_into().unwrap());
    ///
    /// let mut iter = map.into_iter();
    /// assert_eq!(iter.next(), Some((Some(ContentLength), "123".try_into().unwrap())));
    /// assert_eq!(iter.next(), Some((Some(ContentType), "application/sdp".try_into().unwrap())));
    /// assert!(iter.next().is_none());
    /// ```
    ///
    ...
    /// map.append(ContentLength, "456".try_into().unwrap());
    ///
    /// map.append(ContentType, "application/sdp".try_into().unwrap());
    /// map.append(ContentType, "text/parameters".try_into().unwrap());
    /// map.append(ContentType, "application/x-rtsl".try_into().unwrap());
    ///
    /// let mut iter = map.into_iter();
    ///
    /// assert_eq!(iter.next(), Some((Some(ContentLength), "123".try_into().unwrap())));
    /// assert_eq!(iter.next(), Some((None, "456".try_into().unwrap())));
    ///
    /// assert_eq!(iter.next(), Some((Some(ContentType), "application/sdp".try_into().unwrap())));
    /// assert_eq!(iter.next(), Some((None, "text/parameters".try_into().unwrap())));
    /// assert_eq!(iter.next(), Some((None, "application/x-rtsl".try_into().unwrap())));
    /// assert!(iter.next().is_none());
    /// ```
...
impl<T> Extend<(Option<HeaderName>, T)> for HeaderMap<T> {
    ...
    /// let mut map = HeaderMap::new();
    ///
    /// map.insert(Accept, "application/example ;q=0.7, application/sdp".try_into().unwrap());
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    ///
    /// let mut extra = HeaderMap::new();
    ///
    /// extra.insert(Allow, "SETUP".try_into().unwrap());
    /// extra.insert(ContentLength, "2".try_into().unwrap());
    /// extra.append(ContentLength, "1".try_into().unwrap());
    ///
    /// map.extend(extra);
    ///
    /// assert_eq!(map["accept"], "application/example ;q=0.7, application/sdp");
    /// assert_eq!(map["allow"], "SETUP");
    /// assert_eq!(map["content-length"], "2");
    ///
    /// let v = map.get_all("allow");
    /// assert_eq!(1, v.iter().count());
    ///
    /// let v = map.get_all("content-length");
...
#[inline]
fn do_insert_phase_two(indices: &mut Vec<Pos>, mut probe: usize, mut old_pos: Pos) -> usize {
...
impl<'a, T> Entry<'a, T> {
    ...
    ///     .or_insert_with(|| "world".try_into().unwrap());
    ///
    /// assert_eq!(res, "world");
    /// ```
    ///
    ...
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    ///
    /// let res = map.entry("allow")
    ///     .expect("\"allow\" is a valid string")
    ///     .or_insert_with(|| unreachable!());
    ///
    ///
    /// assert_eq!(res, "DESCRIBE");
    /// ```
...
impl<'a, T> VacantEntry<'a, T> {
    ...
    ///
    /// The value will be associated with this entry's key. A mutable reference to the inserted
    ...
    ///
    /// if let Entry::Vacant(v) = map.entry("x-hello").unwrap() {
    ///     v.insert("world".try_into().unwrap());
    /// }
    ///
    ...
    /// Insert the value into the entry.
    ///
    /// The value will be associated with this entry's key. The new `OccupiedEntry` is returned,
    ...
    ///
    /// if let Entry::Vacant(v) = map.entry("x-hello").unwrap() {
    ///     let mut e = v.insert_entry("world".try_into().unwrap());
    ///     e.insert("world2".try_into().unwrap());
    /// }
    ///
...
impl<'a, T: 'a> GetAll<'a, T> {
    ...
    ///
    /// assert_eq!(&"DESCRIBE", iter.next().unwrap());
    /// assert_eq!(&"SETUP", iter.next().unwrap());
    /// assert!(iter.next().is_none());
    /// ```
...
impl<'a, T> OccupiedEntry<'a, T> {
    ...
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    ///
    /// if let Entry::Occupied(e) = map.entry("allow").unwrap() {
    ///     assert_eq!("allow", e.key());
    /// }
    /// ```
    pub fn key(&self) -> &HeaderName {
    ...
    /// map.insert(Allow, "DESCRIBE".try_into().unwrap());
    ///
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     assert_eq!(e.get(), &"DESCRIBE");
    ///
    ///     e.append("SETUP".try_into().unwrap());
    ///
    ///     assert_eq!(e.get(), &"DESCRIBE");
    /// }
    /// ```
    ...
    ///
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     e.get_mut().push_str(", SETUP");
    ///     assert_eq!(e.get(), &"DESCRIBE, SETUP");
    /// }
    /// ```
    ...
    ///
    /// if let Entry::Occupied(e) = map.entry("allow").unwrap() {
    ///     e.into_mut().push_str(", GET_PARAMETER");
    /// }
    ///
    /// assert_eq!("DESCRIBE, GET_PARAMETER", map["allow"]);
    /// ```
    ...
    /// `insert_mult` for an API that returns all values.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     let mut prev = e.insert("SETUP".try_into().unwrap());
    ///     assert_eq!("DESCRIBE", prev);
    /// }
    ///
    /// assert_eq!("SETUP", map["allow"]);
    /// ```
    ...
    /// Sets the value of the entry.
    ///
    /// This function does the same as `insert` except it returns an iterator that yields all values
    ...
    ///
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     let mut prev = e.insert_mult("GET_PARAMETER".try_into().unwrap());
    ///     assert_eq!("DESCRIBE", prev.next().unwrap());
    ///     assert_eq!("SETUP", prev.next().unwrap());
    ///     assert!(prev.next().is_none());
    /// }
    ///
    /// assert_eq!("GET_PARAMETER", map["allow"]);
    /// ```
    ...
    /// Insert the value into the entry.
    ///
    /// The new value is appended to the end of the entry's value list. All previous values
    /// associated with the entry are retained.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     e.append("DESCRIBE".try_into().unwrap());
    /// }
    ///
    /// let values = map.get_all("allow");
    /// let mut i = values.iter();
    ...
    /// `remove_entry_mult` for an API that returns all values.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Remove the entry from the map.
    ///
    /// The key and all values associated with the entry are removed and the first one is returned.
    /// See `remove_entry_mult` for an API that returns all values.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// if let Entry::Occupied(mut e) = map.entry("allow").unwrap() {
    ///     for e in e.iter_mut() {
    ///         e.push_str(", GET_PARAMETER");
    ///     }
    /// }
    ///
    /// let mut values = map.get_all("allow");
    /// let mut i = values.iter();
    /// assert_eq!(&"SETUP, GET_PARAMETER", i.next().unwrap());
    /// assert_eq!(&"DESCRIBE, GET_PARAMETER", i.next().unwrap());
    /// ```
...
mod into_header_name {
    ...
    // All methods are on this pub(super) trait, instead of `IntoHeaderName`, so that they are not
    // publicly exposed to the world.
    //
    // Being on the `IntoHeaderName` trait would mean users could call
    // `"allow".insert(&mut map, "localhost")`.
    //
...
mod as_header_name {
    ...
    // All methods are on this pub(super) trait, instead of `AsHeaderName`, so that they are not
    // publicly exposed to the world.
    //
    // Being on the `AsHeaderName` trait would mean users could call `"allow".find(&map)`.
    //
    // Ultimately, this allows us to adjust the signatures of these methods without breaking any
    // external crate.
    pub trait Sealed {
        #[doc(hidden)]
